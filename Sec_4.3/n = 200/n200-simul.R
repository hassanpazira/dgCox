...
set.seed(112358)                            # setting the seed for the random number generator
...
##########################
## Output
results <- array(0, dim = c(nsim, length(p), length(s), 6, 11),
	dimnames = list(nsim = 1:nsim, p = p, s = s,
	gof = c("AIC", "BIC", "Fan_13", "GIC_aic", "GIC_bic", "GIC_bic_modified"),
...
for(h in 1:length(p)){
	...
	for(k in length(s):1){
		...
		repeat{
			...
			out.dgcoxph <- try(dgcoxphR(tm, status, X, g0 = g0[h], maxrsq_ratio = 1, np = np[h], verbose = FALSE), TRUE)
			...
			if(class(out.gof) != "try-error"){
				...
				######################################
				# GIC_bic_modified section
				GIC_bic_modified <- gof(out.dgcoxph, k = k.gof, complexity = "gic")
				id <- which.min(GIC_bic_modified)
				results[i, h, k, "GIC_bic_modified", "value"] <- GIC_bic_modified[id]
				results[i, h, k, "GIC_bic_modified", "size"] <- df[id]
				results[i, h, k, "GIC_bic_modified", "TP"] <- sum(bh[A, id] != 0)
				results[i, h, k, "GIC_bic_modified", "FP"] <- sum(bh[-A, id] != 0)
				results[i, h, k, "GIC_bic_modified", "TN"] <- (p[h] - s[k]) - sum(bh[-A, id] != 0)
				results[i, h, k, "GIC_bic_modified", "FN"] <- s[k] - sum(bh[A, id] != 0)
				results[i, h, k, "GIC_bic_modified", "TPR"] <- sum(bh[A, id] != 0) / s[k]
				results[i, h, k, "GIC_bic_modified", "FPR"] <- sum(bh[-A, id] != 0) / (p[h] - s[k])
				results[i, h, k, "GIC_bic_modified", "FDR"] <- sum(bh[-A, id] != 0) / (sum(bh[A, id] != 0) + sum(bh[-A, id] != 0))
				results[i, h, k, "GIC_bic_modified", "FNR"] <- 1 - (sum(bh[A, id] != 0) / s[k])
				results[i, h, k, "GIC_bic_modified", "F1"] <- 2 * sum(bh[A, id] != 0) / (s[k] + sum(bh[A, id] != 0) + sum(bh[-A, id] != 0))
				
				#*************************************
				cat("Simulation", i, "with p =", p[h], "and s =", s[k], "completed\n")
				if(i %in% save.point) save.image(file = "gof-n200_tmp.RData")
			} else
