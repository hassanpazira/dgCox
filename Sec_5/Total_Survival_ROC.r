...
## Ovarian
data   = read.table("GSE30009_series_matrix.txt",row.names = 1, sep = "\t");rownames(data)[1] <- c("time")
## Skin
data = read.table("GSE22153_series_matrix.txt",row.names = 1, sep = "\t")
## Prostate
data = read.table("GSE45705_series_matrix_Edited.txt",row.names = 1, sep = "\t")
## Colon
...
# Setting
set.seed(11235)
id <- sample(2, n, replace=TRUE, prob=c(1.0, 0.0))  # c(0.6, 0.4)
...
## run procedure
out.dgcoxph.train  <- dgcoxphR(time_train, censor_train, X[train,],g0=0.5,verbose=T)
...
###################################################
# we need to get a dataset in the following format:
# 4 (or more) columns indicating
# - time (event time or censoring time)
# - censoring indicator
# - score for the various prediction models 
#   In practice these can be the linear predictors of the log hazard in the various models
# - predict.time 
...
ovarian.ROC$AUC
#str(ovarian.ROC)
#plot(ovarian.ROC$FP,ovarian.ROC$TP,type="o")

# NNE & lambda=0.05
ovarian.ROC.NNE<-survivalROC(time_train, censor_train, W_train, predict.time = tm, lambda=0.05)
ovarian.ROC.NNE$AUC
#lines(ovarian.ROC.NNE$FP,ovarian.ROC.NNE$TP,lty=2)
#abline(0,1)
#legend("bottomright",c("KM", "NNE_lam_0.05", "NNE_lam_0.01"),lty=c(1,2,3))
...
#out.cv.pen.train<-cvl(Surv(time_train, censor_train), penalized = X[train,], lambda1 = 1, model ="cox" )
...
# sapply(out.penal, coefficients, "p")
...
#### Final Plot
pdf("ovarian-survival-roc.pdf",width=6,height=6)
...
ovarian.ROC.pen$AUC
title("Ovarian survival ROC")
#lines(c(0,1),c(0,1),lty=2,lwd=2)
legend("bottomright",c(paste( "dgCox ( AUC=",round(ovarian.ROC.NNE$AUC,3),")"), paste( "glmnet ( AUC=",round(ovarian.ROC.glmnet$AUC,3),")"), paste( "glmpath ( AUC=",round(ovarian.ROC.glmpath$AUC,3),")"), paste( "penalized ( AUC=",round(ovarian.ROC.pen$AUC,3),")")),lty=c(1,2,3,4))
dev.off()
