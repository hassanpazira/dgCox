...
# define elemnets
X = apply(as.matrix(data[sel,3:dim(data)[2]]),2, as.numeric)
X = X[,colMeans(is.na(X)) == 0] 
time = as.numeric(levels(data[sel,1]))[data[sel,1]]
censor = as.numeric(levels(data[sel,2]))[data[sel,2]]
###############################################################
...
#ovarian.ROC.glmnet<-survivalROC.C(time_train, censor_train, W_glmnet, predict.time = tm)
...
#ovarian.ROC.glmpath<-survivalROC.C(time_train, censor_train, W_glmpath, predict.time = tm)
...
#ovarian.ROC.pen<-survivalROC.C(time_train, censor_train, W_pen, predict.time = tm)
ovarian.ROC.pen$AUC


#### Final Plot
pdf("Prostate-survival-roc.pdf",width=6,height=6)
